---
layout: hide
lock: d0a4b3d376be0af335a4d748e48327cbc6a4fd9e12dbfef2cea690cf2393319f65ff1deef529a0bf49826c408ba4c757788bcf4cf9ad7b7a85ea018a57540087
salt: f862f1d08bf480f0a6c1f8db1914330976935c513eb154813220bad9fccad8a53f96d6ca0a12cead5d5fb2a65cbbd6e27d3b31d622f38c731e18de1d669df512d565ecea212aae
---

Table of News
====

<div><label id="nowTime">00:00:00</label> </div>
<script>
    const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    function showTime() {
      const currentDate = new Date();
      const options = { timeZone: timeZone, hour: '2-digit', minute: '2-digit', second: '2-digit'};
      const timestamp = currentDate.toLocaleString('en-US', options);
      let labelElement = document.getElementById("nowTime");
      labelElement.innerHTML = timestamp;
    }    
    setInterval(showTime, 1000);
    showTime();
</script>

<div id="searchF">	
    <input type="text" id="searchInput" placeholder="Enter text to search...">
    <button id="searchButton">Search</button>
</div>

<script>
    let searchT = "";
    document.getElementById("searchF").style.display = "none";
	document.getElementById("nowTime").onclick = function() {
		if ( document.getElementById("searchF").style.display === "none"){
			document.getElementById("searchF").style.display = "block";
		}else{
			document.getElementById("searchF").style.display = "none";
		}
	};
    
    // 搜尋按鈕點擊事件
    document.getElementById('searchButton').addEventListener('click', function() {
        const searchText = document.getElementById('searchInput').value.trim();
        if (searchText) {
            searchT = searchText;
            performSearch(searchT, false); // 開始新的搜尋
        }
    });

    // 監聽 Enter 鍵找下一個
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
            const searchText = document.getElementById('searchInput').value.trim();
            if (searchText) {
                // 如果搜尋文字變了，重新開始；否則找下一個
                if (searchText !== searchT) {
                    searchT = searchText;
                    performSearch(searchT, false);
                } else {
                    performSearch(searchT, true);
                }
                event.preventDefault(); // 防止表單提交或換行
            }
        }
    });

    /**
     * 核心搜尋函數
     * @param {string} text - 要搜尋的文字
     * @param {boolean} next - 是否搜尋下一個
     */
    function performSearch(text, next) {
        // 使用 window.find 是最簡單且支援「找下一個」的瀏覽器原生方法
        // 參數說明: (字串, 是否區分大小寫, 是否向後搜尋, 是否循環搜尋, 是否全字匹配, 是否搜尋框, 是否向前搜尋)
        const found = window.find(text, false, false, true, false, true, false);

        if (found) {
            // 取得目前選中的區域並確保它在視線內
            const selection = window.getSelection();
            if (selection.rangeCount > 0) {
                const range = selection.getRangeAt(0);
                const element = range.startContainer.parentElement;
                
                // 視覺回饋：讓找到的文字閃爍或顯示
                element.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
        } else {
            // 如果到底了，再跑一次會從頭開始（因為上面設置了循環搜尋 true）
            // 或者你可以給予提示
            console.log('已到達文件末尾，將從頭開始搜尋。');
        }
    }
</script>

{% assign pp = site.pages | sort_natural: "title" %}
{% for p in pp %} {% if p.title %}{% if p.layout == "movie" %}🎥[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "s" %}🔄[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "shorturl" %}🔄[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "jump" %}🔄[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "lockjump" %}🔒🔄[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "default" %} 🔹[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "listpage" %}.[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "movieList" %}.[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "myMdShow" %}.[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "pt" %}🔒📄[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "empty" %}🈳[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "hide" %}🔒[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "lockhtml" %}🔒[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "lock" %}🔒[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "encpara" %}🔏[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "lockurl" %}🔒[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "zip" %}🔒[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "enc" %}🔏[{{ p.title }}  ]({{ p.url }}) 
{% elsif p.layout == "" %}x-[{{ p.title }}  ]({{ p.url }}) 
{% else %}xx-{{ p.layout }} [{{ p.title }}  ]({{ p.url }}) 
{% endif %}{% endif %}{% endfor %}

# Movie
{% assign pp = site.pages | sort_natural: "title" %}{% for p in pp %} {% if p.title %}{% if p.layout == "movie" %}[{{ p.title }}  ]({{ p.url }}) 
{% endif %}{% endif %}{% endfor %}


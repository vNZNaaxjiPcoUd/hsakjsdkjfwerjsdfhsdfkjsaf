...
# 好，這個情境其實**很單純**，而且對你是「好消息」。我直接用你的條件一步一步說清楚 👇

> **條件回顧**
>
> * 2025 年收入：約 **$12,000**
> * 聯邦 **應稅額 = $0**
> * 購車時 **已使用 Point of Sale (POS) Transfer**
> * 經銷商已即時幫你折抵（例如 $7,500）
...
**你報稅時「不需要退回任何錢」，也「不會因為稅額是 0 而出問題」**。
你只需要在報稅時 **正確申報「已轉讓（Transferred）」這筆 Clean Vehicle Credit** 即可。
...
👉 **Form 8936 還是一定要附上**
（這是 IRS 用來「對帳」你跟經銷商的紀錄）

---

### 2️⃣ 在 Form 8936 裡怎麼填？（重點欄位）

在 **Form 8936（2025 版）** 裡，你會看到跟 **Transfer** 有關的欄位：

* ✔️ 勾選你 **選擇將 Clean Vehicle Credit 轉讓給經銷商**
* ✔️ 填入：
...
📌 **關鍵點**：
這筆 credit **不會再進入你的 tax liability 計算**
它只是被「標示為已轉讓、已在銷售時使用」
...
| Credit 使用方式           | **Transferred at Point of Sale** |
| 對稅額的影響                | **0（不再計算）**                      |
| 需退回 IRS？              | ❌ 不需要                            |
| IRS 會追討？              | ❌ 不會                             |
...
依照 **IRC §30D + IRS Transfer FAQ** 的規定：

> **只要你在購車時合法選擇 POS Transfer，
> 即使你當年度的稅額不足（甚至是 0），
> IRS 也「不得」要求你補繳或退回差額。**

👉 風險是在 **經銷商那邊**，不是在你身上
（如果車不合格，是 IRS 跟 dealer 算帳）
...
1. **你的 MAGI 超過收入上限**（你 $12,000 → 完全不可能）
2. 車其實不符合 §30D，但你有刻意提供錯誤資訊
3. VIN / 經銷商申報資料不一致（這通常是 dealer 的問題）

👉 正常買車、正常報稅，**你這種低收入 + POS Transfer 是最穩的一種情況**
